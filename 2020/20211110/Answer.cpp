...
namespace hpc {
	...
	void Answer::initialize(const Stage& aStage)
	{
		...
		_RelayX.push_back( int(aStage.rabbit().pos().x) );
		_RelayY.push_back( int(aStage.rabbit().pos().y) );

		for (auto scroll : aStage.scrolls()) {
			_RelayX.push_back(int(scroll.pos().x));
			_RelayY.push_back(int(scroll.pos().y));
			++scrNum;
		}
		// ウサギの分も追加
		scrNum += 1;
		...
		for (auto i = 0; i < int(_RelayX.size()); ++i) {
			_RelayList.push_back(Vector2(float(_RelayX[i]), float(_RelayY[i])));
		...
		for (int srcIdx = 0; srcIdx < _RelayList.size() ; ++srcIdx) {
			for (int tgtIdx = srcIdx; tgtIdx < _RelayList.size() ; ++tgtIdx) {
				...
				for (int getScrNumIdx = 0; getScrNumIdx < SCROLL_NUM; ++getScrNumIdx) {
					...
					// 同じ座標だったら無視
					if (_RelayX[srcIdx] == _RelayX[tgtIdx] &&
						_RelayY[srcIdx] == _RelayY[tgtIdx]) {
						continue;
					}
	...
	void Answer::addRelayPointOfPondCorner(const Stage& aStage, int aX, int aY)
	{
		...
		if (aStage.terrain(*checkedTerrainLeftUp) == hpc::Terrain::Pond &&
			...
			_RelayX.push_back(aX + 1);
			_RelayY.push_back(aY + 1);
			_RelayList.push_back(Vector2(aX + 1, aY + 1));
		}
		// ２．4マス調べて、右上だけ水のパターン
		if (aStage.terrain(*checkedTerrainLeftUp) != hpc::Terrain::Pond &&
			aStage.terrain(*checkedTerrainLeftDown) != hpc::Terrain::Pond &&
			...
			_RelayX.push_back(aX);
			_RelayY.push_back(aY + 1);
			_RelayList.push_back(Vector2(aX, aY + 1));
		}
		// ３．4マス調べて、左下だけ水のパターン
		if (aStage.terrain(*checkedTerrainLeftUp) != hpc::Terrain::Pond &&
			aStage.terrain(*checkedTerrainLeftDown) == hpc::Terrain::Pond &&
			...
			_RelayX.push_back(aX + 1);
			_RelayY.push_back(aY);
			_RelayList.push_back(Vector2(aX + 1, aY));
		}
		// ４．4マス調べて、右下だけ水のパターン
		if (aStage.terrain(*checkedTerrainLeftUp) != hpc::Terrain::Pond &&
			aStage.terrain(*checkedTerrainLeftDown) != hpc::Terrain::Pond &&
			...
			_RelayX.push_back(aX);
			_RelayY.push_back(aY);
			_RelayList.push_back(Vector2(aX, aY));
		}
	}
		...
		for (auto scroll : aStage.scrolls()) {
			...
			while (1) {
				...
				if (aStage.terrain(ResearchTerrain) != hpc::Terrain::Pond) {
					// 水/陸の境界を中継点として追加
					int x = int(ResearchTerrain.x);
					int y = int(ResearchTerrain.y);
					
					_RelayX.push_back(x);
					_RelayY.push_back(y);
					_RelayList.push_back(Vector2(float(x), float(y)));
					break;
				}
			}
			// ２．巻物の下方向の水/陸の境界を探る
