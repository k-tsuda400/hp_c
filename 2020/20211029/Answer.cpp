...
namespace hpc {
	...
	void Answer::initialize(const Stage& aStage)
	{
		...
		for (auto i = 0; i < STAGE_WIDTH - 1; ++i) {
			for (auto j = 0; j < STAGE_HEIGHT - 1; ++j) {
				...
				if (aStage.terrain(*checkedTerrainLeftUp) == hpc::Terrain::Pond &&
					...
					relayList.push_back(Vector2(i + 1, j + 1));
				}
			}
		}
		...
		vector<vector<vector<int>>> simuWalkNumList(relayList.size(), vector<vector<int>>(relayList.size(), vector<int>(SCROLL_NUM, 999999)));

		for (int srcIdx = 0; srcIdx < relayList.size() ; ++srcIdx) {
			for (int tgtIdx = srcIdx; tgtIdx < relayList.size() ; ++tgtIdx) {
				// 取得巻物数ごとに変化
		...
		for (auto startScr = 0; startScr < SCROLL_NUM-1; ++startScr) {
			calcRelayFromDijkstra(startScr, orderGetScr);
		}
	...
	void Answer::calcRelayFromDijkstra(int startScrNum, std::vector<int> vec)
	{
		// とりあえず関数の大枠だけ作って中身は空

		// ↓これ呼び出すとなぜかvectorの例外エラーに引っかかる
		//_OrderGetScrWithRelay.push_back(vec[startScrNum]);
		//_OrderGetScrWithRelay.push_back(vec[startScrNum + 1]);
	}
