...
namespace hpc {
	...
	void Answer::initialize(const Stage& aStage)
	{
		...
		if (greedySimuWalk <= greedySimuWalk2nd) {
			...
			// orderGetScrをorgerGetScr2ndにコピー
			orderGetScr2nd.clear();

			for (auto i = 0; i < orderGetScr.size(); ++i) {
				orderGetScr2nd.push_back(orderGetScr.at(i));
			}
		}

		// 2-optと山登り法を合体し、
		// 2つの巻物取る順をランダムに入れ替えて、改善されたら採用する処理
		int sumWalkCost2opt = (greedySimuWalk <= greedySimuWalk2nd) ?
			greedySimuWalk : greedySimuWalk2nd;

		if (SCROLL_NUM < 6) {
			// 巻物数が少ない場合は何もしない
		}
		else {
			for (auto executeNum = 0; executeNum < 10000; ++executeNum) {
				// ここで入れ替え処理
				std::mt19937 get_rand_mt(executeNum); // seedを指定
				int randIdx1 = get_rand_mt() % (SCROLL_NUM - 2) + 1;
				int randIdx2 = get_rand_mt() % (SCROLL_NUM - 2) + 1;
				// 配列要素入れ替え
				// 参考：https://cpprefjp.github.io/reference/algorithm/iter_swap.html
				std::iter_swap(orderGetScr2nd.begin() + randIdx1,
							   orderGetScr2nd.begin() + randIdx2);

				// このループ内で合計ターン数を計算する
				int sumWalkCostTmp = 0;
				for (auto scrNum = 0; scrNum < SCROLL_NUM - 1; ++scrNum) {
					if (scrNum == 0) {
						sumWalkCostTmp += simuWalkNumList.at(0).at(orderGetScr2nd[scrNum] + 1).at(scrNum);
					}
					else {
						sumWalkCostTmp += simuWalkNumList.at(orderGetScr2nd[scrNum - 1] + 1).at(orderGetScr2nd[scrNum] + 1).at(scrNum);
					}
				}

				// もし2optした結果がよくなったら、そちらを採用
				if (sumWalkCostTmp < sumWalkCost2opt) {
					// 最適歩数を更新
					sumWalkCost2opt = sumWalkCostTmp;
				}
				else {
					// 悪化した場合はその結果を使いたくないので、入れ替えた配列を元に戻す
					std::iter_swap(orderGetScr2nd.begin() + randIdx1,
									orderGetScr2nd.begin() + randIdx2);
				}
			}
		...
		// -------
		// 最後の仕上げに、中継点を含めた状態で2-optを実行
		// sumWalkCost2opt 変数はTSP時の2-optから流用
		...
		if (SCROLL_NUM < 6) {
			...
			for (auto executeNum = 0; executeNum < 10000; ++executeNum) {
				...
				int randIdx2 = get_rand_mt() % (SCROLL_NUM - 2) + 1;
				// 配列要素入れ替え
				std::iter_swap(_OrderGetScrWithRelay.begin() + randIdx1,
							   _OrderGetScrWithRelay.begin() + randIdx2);

				// このループ内で合計ターン数を計算する
				int sumWalkCostTmp = 0;
				for (auto scrNum = 0; scrNum < SCROLL_NUM - 1; ++scrNum) {
					if (scrNum == 0) {
						sumWalkCostTmp += simuWalkNumList.at(0).at(_OrderGetScrWithRelay[scrNum] + 1).at(scrNum);
					}
					else {
						sumWalkCostTmp += simuWalkNumList.at(_OrderGetScrWithRelay[scrNum - 1] + 1).at(_OrderGetScrWithRelay[scrNum] + 1).at(scrNum);
					}
				}
