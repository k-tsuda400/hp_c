...
namespace hpc {
	...
	void Answer::initialize(const Stage& aStage)
	{
		...
		for (auto startScr = 0; startScr < SCROLL_NUM-1; ++startScr) {
			...
			isYamanobori ? calcRelayFromDijkstra(startScr, orderGetScr, simuWalkNumList) :
						   calcRelayFromDijkstra(startScr, orderGetScr2nd, simuWalkNumList);
		}
	...
	void Answer::calcRelayFromDijkstra(const int startScrNum, const std::vector<int> vec, const vector<vector<vector<int>>> walkCostList)
	{
		...
		int minWalkCost;
		if (startScrNum == 0) {
			defaultWalkCost = walkCostList.at(0).at(vec[startScrNum] + 1).at(startScrNum);
		}
		else {
			defaultWalkCost = walkCostList.at(vec[startScrNum-1] + 1).at(vec[startScrNum] + 1).at(startScrNum);
		}

		minWalkCost = defaultWalkCost;
		for (int i = 0; i < _RelayList.size(); ++i) {
			int relayWalkCost = walkCostList.at(0).at(i).at(startScrNum) +
								walkCostList.at(i).at(vec[startScrNum] + 1).at(startScrNum);
	...
	Vector2 Answer::getTargetPos(const Stage& aStage)
	{
		...
		if (rabPos.x == _RelayList[_OrderGetScrWithRelay[nowTarget] + 1].x &&
			rabPos.y == _RelayList[_OrderGetScrWithRelay[nowTarget] + 1].y) {
			++nowTarget;
			return _RelayList[_OrderGetScrWithRelay[nowTarget - 1] + 1];
		}
		else {
			if (nowTarget >= _OrderGetScrWithRelay.size()) {
				--nowTarget;
			}
			return _RelayList[_OrderGetScrWithRelay[nowTarget] + 1];
		}
	}
