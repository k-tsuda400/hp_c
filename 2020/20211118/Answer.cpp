...
namespace hpc {
	...
	void Answer::initialize(const Stage& aStage)
	{
		...
		*/
		...
		// 配列をシャッフルするパターン
		vector<int> randomScrOrderList;
		for (auto i = 0; i < SCROLL_NUM - 1; ++i){
			randomScrOrderList.push_back(i);
		}
		...
		int executeNumMax;
		if (SCROLL_NUM - 1 < 7) {
			executeNumMax = 27 * (SCROLL_NUM - 1) * (SCROLL_NUM - 1);
		}
		else {
			executeNumMax = 10000 * (SCROLL_NUM - 1) * (SCROLL_NUM - 1);
		}

		for (auto executeNum = 0; executeNum < executeNumMax ; ++executeNum) {
			std::mt19937 get_rand_mt(executeNum); // seedを指定
			std::shuffle(randomScrOrderList.begin(), randomScrOrderList.end(), get_rand_mt);
		
			// このループ内で合計ターン数を計算する
			int sumWalkCostTmp = 0;
			for (auto scrNum = 0; scrNum < SCROLL_NUM - 1; ++scrNum) {
				if (scrNum == 0) {
					sumWalkCostTmp += simuWalkNumList.at(0).at(randomScrOrderList[scrNum]+1).at(scrNum);
				}
				else {
					sumWalkCostTmp += simuWalkNumList.at(randomScrOrderList[scrNum-1]+1).at(randomScrOrderList[scrNum]+1).at(scrNum);
				}
			}

			if (sumWalkCostTmp < greedySimuWalk2nd) {
				greedySimuWalk2nd = sumWalkCostTmp;
				orderGetScr2nd.clear();
				for (auto cpyIdx = 0; cpyIdx < SCROLL_NUM - 1; ++cpyIdx) {
					orderGetScr2nd.push_back(randomScrOrderList[cpyIdx]);
				}
				//int a = 1;
			}
		}
