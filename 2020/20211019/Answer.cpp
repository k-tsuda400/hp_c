	...
	void Answer::initialize(const Stage& aStage)
	{
		...
		// 普通の貪欲法の歩数をシミュレート
		{
			...
			for (auto i = 0; i < SCROLL_NUM; ++i) {
				...
				// 最初の1ターン
				if (i == 0) {
					int tekitoNum = int(SCROLL_NUM / 2);
					minWalkCost = simuWalkNumList.at(nextTargetScr).at(tekitoNum);
				...
				if (minWalkCost != INVALID_BIG_INT) {
					nextTargetScr = nextTargetScrKouho;
					greedySimuWalk2nd += minWalkCost;
					debug2ndGreedySimu.push_back(minWalkCost);
					orderGetScr2nd.push_back(nextTargetScr - 1);
					relayList_isPassed.at(nextTargetScr) = true;
				}
			}
